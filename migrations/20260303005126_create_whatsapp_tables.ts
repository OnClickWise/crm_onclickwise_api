...
export async function up(knex: Knex): Promise<void> {
    ...
    const hasWhatsappAccounts = await knex.schema.hasTable('whatsapp_accounts');
    if (!hasWhatsappAccounts) {
      ...
      console.log('Tabela "whatsapp_accounts" já existe, pulando criação');
    }

    const hasWhatsappConversations = await knex.schema.hasTable('whatsapp_conversations');
    if (!hasWhatsappConversations) {
      ...
      console.log('Tabela "whatsapp_conversations" já existe, pulando criação');
    }

    const hasWhatsappMessages = await knex.schema.hasTable('whatsapp_messages');
    if (!hasWhatsappMessages) {
      ...
      console.log('Tabela "whatsapp_messages" já existe, pulando criação');
    }
}
