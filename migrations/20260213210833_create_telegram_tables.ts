...
export async function up(knex: Knex): Promise<void> {
  ...
  const hasTelegramBots = await knex.schema.hasTable('telegram_bots');
  if (!hasTelegramBots) {
    ...
    console.log('Table "telegram_bots" already exists, skipping creation');
  }

  // 2. Create telegram_accounts table
  const hasTelegramAccounts = await knex.schema.hasTable('telegram_accounts');
  if (!hasTelegramAccounts) {
    ...
    console.log('Table "telegram_accounts" already exists, skipping creation');
  }

  // 3. Create telegram_account_sessions table
  const hasTelegramAccountSessions = await knex.schema.hasTable('telegram_account_sessions');
  if (!hasTelegramAccountSessions) {
    ...
    console.log('Table "telegram_account_sessions" already exists, skipping creation');
  }

  // 4. Create telegram_conversations table
  const hasTelegramConversations = await knex.schema.hasTable('telegram_conversations');
  if (!hasTelegramConversations) {
    ...
    console.log('Table "telegram_conversations" already exists, skipping creation');
  }

  // 5. Create telegram_messages table
  const hasTelegramMessages = await knex.schema.hasTable('telegram_messages');
  if (!hasTelegramMessages) {
    ...
    console.log('Table "telegram_messages" already exists, skipping creation');
  }
    
    // Removido: linha solta fora do bloco da função createTable
}
